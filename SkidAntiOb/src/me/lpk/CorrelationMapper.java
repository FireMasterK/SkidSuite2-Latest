...
public class CorrelationMapper {
	...
	public static Map<String, MappedClass> correlate(MappedClass targetClass, MappedClass cleanClass, Map<String, MappedClass> targetMap,
		...
		int offsetField = 0, offsetField2 = 0;
		for (int key = 0; key < targetFields.size(); key++) {
			if (key >= cleanFields.size()) {
				continue;
			}
			MappedMember targetField = targetFields.get(key + offsetField2);
			MappedMember cleanField = cleanFields.get(key + offsetField);
			if (targetClass.getOriginalName().equals("mm")){
			//	System.out.println("TESTING:" + cleanField.getOriginalName() + ":" + targetField.getOriginalName());
			}
			// Extra field?
			if (cleanField == null) {
				continue;
			...
			if (!fix(targetField.getDesc()).equals(fix(cleanField.getDesc()))) {
				...
				// x++
				//System.out.println("       NOT-SMPL: " + cleanField.getOriginalName() + ":" + targetField.getOriginalName());
				offsetField -= 1;
				continue;
			} else {
				if (targetField.getDesc().length() > 4 && cleanField.getDesc().length() > 4) {
					...
					if (c1 != null && c2 != null) {
						if (!areSimiliar(c1, c2)) {
							//System.out.println("       NOT-CMPL: " + cleanField.getOriginalName() + ":" + targetField.getOriginalName());
							offsetField2 -= 1;
							continue;
						}
					}
